...
Raw burst data structure: 
  ...
  BYTE[07] = Raw_Data_Sum   = It reports the upper byte of an 18�륿it counter which sums all 1296 raw data in the current frame;
...
- PMW3389_DATA.SQUAL         : byte, Surface Quality register, max 0x80
                       * Number of features on the surface = SQUAL * 8
- PMW3389_DATA.rawDataSum    : byte, It reports the upper byte of an 18�륿it counter 
...
struct PMW3389_DATA
{
 ...
 byte rawDataSum;      // It reports the upper byte of an 18�륿it counter which sums all 1296 raw data in the current frame; * Avg value = Raw_Data_Sum * 1024 / 1296
